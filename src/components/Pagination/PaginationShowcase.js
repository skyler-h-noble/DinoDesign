...
export function PaginationShowcase() {
  ...
  const generateCode = () => {
    ...
    if (count !== 10) parts.push('count={' + count + '}');
    if (showFirst) parts.push('showFirstButton');
    if (showLast) parts.push('showLastButton');
    if (disabled) parts.push('disabled');
    parts.push('onChange={handlePageChange}');
    if (parts.length === 1) return '<Pagination onChange={handlePageChange} />';
    return '<Pagination\n  ' + parts.join('\n  ') + '\n/>';
  };
            ...
            <Tabs defaultValue={0} variant="standard" color="primary">
              ...
              <TabPanel value={0}>
                <Box sx={{ p: 3 }}>
                  ...
                  {/* Pages */}
                  <Box sx={{ mt: 3 }}>
                    <OverlineSmall style={{ color: 'var(--Text-Quiet)', display: 'block', marginBottom: 8 }}>PAGES</OverlineSmall>
                    <Stack direction="row" spacing={1}>
                      {[5, 10, 20, 50].map((n) => (
                        <ControlButton key={n} label={String(n)} selected={count === n} onClick={() => setCount(n)} />
                      ))}
                    </Stack>
                  </Box>
                  ...
                  <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box>
                      ...
                      <Caption style={{ color: 'var(--Text-Quiet)', display: 'block' }}>Show jump to first/last page</Caption>
                    </Box>
                    <Switch variant="default-outline" checked={showFirst}
                      onChange={(e) => { setShowFirst(e.target.checked); setShowLast(e.target.checked); }}
                      size="small" aria-label="First/last buttons" />
                  </Box>
                ...
                <Box sx={{ p: 3 }}>
                  <Stack spacing={3}>
                    <Box sx={{ p: 3, backgroundColor: 'var(--Background)', borderRadius: 'var(--Style-Border-Radius)', border: '1px solid var(--Border)' }}>
                      ...
                      <Stack spacing={0}>
                        {[
                          { label: 'Nav',       value: '<nav aria-label="Pagination">' },
                          { label: 'List',      value: '<ol> with <li> for each button' },
                          { label: 'Current',   value: 'aria-current="page" on selected page' },
                          { label: 'Labels',    value: 'aria-label="Page N", "Go to previous/next/first/last page"' },
                          { label: 'Keyboard',  value: 'Tab navigates between buttons' },
