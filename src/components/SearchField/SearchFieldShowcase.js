...
export function SearchFieldShowcase() {
  ...
  const generateCode = () => {
    ...
    parts.push('placeholder="Search..."');
    parts.push('onChange={handleSearch}');
    return '<SearchField\n  ' + parts.join('\n  ') + '\n/>';
  };
                ...
                <Box sx={{ p: 3 }}>
                  ...
                  <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box>
                      ...
                      <Caption style={{ color: 'var(--Text-Quiet)', display: 'block' }}>Show clear when has value</Caption>
                    </Box>
                    <Switch variant="default-outline" checked={showClear} onChange={(e) => setShowClear(e.target.checked)}
                      size="small" aria-label="Clear button" />
                  </Box>
                ...
                <Box sx={{ p: 3 }}>
                  <Stack spacing={3}>
                    ...
                    <Box sx={{ p: 3, backgroundColor: 'var(--Background)', borderRadius: 'var(--Style-Border-Radius)', border: '1px solid var(--Border)' }}>
                      ...
                      <Stack spacing={0}>
                        {[
                          { label: 'Role',     value: 'role="searchbox" on the input element' },
                          { label: 'Label',    value: 'aria-label="Search" (configurable via ariaLabel prop)' },
                          { label: 'Clear',    value: 'aria-label="Clear search" on clear button' },
                          { label: 'Keyboard', value: 'Enter submits, Escape clears' },
