...
export function CheckboxShowcase() {
  ...
  const generateCode = () => {
    ...
    if (showLabel && labelText) parts.push('label="' + labelText + '"');
    if (checked) parts.push('checked');
    if (indeterminate) parts.push('indeterminate');
    if (disabled) parts.push('disabled');
    if (!showLabel) parts.push('aria-label="Checkbox"');
    return '<Checkbox\n  ' + parts.join('\n  ') + '\n/>';
  };
        ...
        <Grid item sx={{ width: { xs: '100%', md: '55%' }, flexShrink: 0, pr: { md: 3 } }}>
          ...
          <PreviewSurface theme={bgTheme} surface={bgSurface}>
            <Checkbox
              ...
              disabled={disabled}
              aria-label={!showLabel ? 'Checkbox' : undefined}
            />
          </PreviewSurface>
            ...
            <Tabs defaultValue={0} variant="standard" color="primary">
              ...
              <TabPanel value={0}>
                <Box sx={{ p: 3 }}>
                  ...
                  {/* State */}
                  <Box sx={{ mt: 3 }}>
                    <OverlineSmall style={{ color: 'var(--Text-Quiet)', display: 'block', marginBottom: 8 }}>STATE</OverlineSmall>
                    <Stack direction="row" spacing={1}>
                      <ControlButton label="Unchecked" selected={!checked && !indeterminate}
                        onClick={() => { setChecked(false); setIndeterminate(false); }} />
                      <ControlButton label="Checked" selected={checked && !indeterminate}
                        onClick={() => { setChecked(true); setIndeterminate(false); }} />
                      <ControlButton label="Indeterminate" selected={indeterminate}
                  ...
                  <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box>
                      ...
                      <Caption style={{ color: 'var(--Text-Quiet)', display: 'block' }}>Text label next to checkbox</Caption>
                    </Box>
                ...
                <Box sx={{ p: 3 }}>
                  <Stack spacing={3}>
                    ...
                    <Box sx={{ p: 3, backgroundColor: 'var(--Background)', borderRadius: 'var(--Style-Border-Radius)', border: '1px solid var(--Border)' }}>
                      ...
                      <Stack spacing={0}>
                        {[
                          { label: 'Role',          value: 'role="checkbox" (native <input type="checkbox">)' },
                          { label: 'Checked',       value: 'aria-checked="true|false|mixed" for indeterminate' },
                          { label: 'Label',         value: 'FormControlLabel associates label with input. Without label, provide aria-label.' },
                          { label: 'Focus',         value: 'outline: 2px solid var(--Focus-Visible); outline-offset: 2px' },
                          { label: 'Disabled',      value: 'HTML disabled attribute. opacity: 0.6, pointer-events: none.' },
                        ].map(({ label, value }) => (
